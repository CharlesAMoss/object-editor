...
// while true you will be returned initial prompt
$inUse = true;
...
do {
  ...
  if ($line == "help" || $line == "HELP") { //HELP command
  ...
  } else if ((preg_match("/^SET\s[a-z][a-zA-Z1-9]*\=[a-zA-Z1-9]*$/", $line))) { //SET propertyname=value command
    $propStr = substr($line, 4);
      if ((preg_match("/^[a-z][a-zA-Z1-9]*\=[a-zA-Z1-9]*$/", $propStr))) {
        $newProp = explode("=", $propStr);
        $person->$newProp[0] = $newProp[1];
  } else if ((preg_match("/^GET\s[a-z][a-zA-Z1-9]*$/", $line))) { //GET propertyname command
      $propStr = substr($line, 4);
      $getProp = "The value of property {$propStr} is {$person->$propStr} \r\n";
      echo $getProp;
  } else if ((preg_match("/^GET\s\*$/", $line))) { //GET * command
      var_dump(get_object_vars($person));
  } else if ($line == "exit" || $line == "EXIT") { //EXIT command
      $inUse = false;
      exit(0);
  } else { // no condtions met, program ends with error message
